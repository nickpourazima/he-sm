...
\begin{figure}[]
    ...
    \caption{Test Case Feedback Example: Red circles are the tap onset, blue squares are the true onset}
...
\begin{itemize}
    ...
    \item Audio latency
    \begin{itemize}
        \item Audio driver and sound card latency estimate
        \begin{itemize}
            \item Tested via loopback method\footnote{\url{https://manual.audacityteam.org/man/latency_test.html}} with audio loopback dongle\footnote{\url{https://source.android.com/devices/audio/latency/loopback}}
        \end{itemize}
        \item Algorithmic latency (pyGame Audio library)
        \begin{itemize}
            \item Sound detector circuit - gate output recorded, discrepancy between time recorded and reported time noted
        \end{itemize}
    \end{itemize}
